...
@app.route('/index')
def index():
    return render_template('index.html',navmenu=navmenu)

# Обработка запроса к corporate
@app.route('/')
@app.route('/corporate')
def corporate():
    return render_template('corporate.html',navmenu=navmenu)

# Обработка запроса к clients
@app.route('/')
@app.route('/clients')
def clients():
    imgs = ['image_01.jpg', 'image_02.jpg','image_03.jpg','image_04.jpg','image_05.jpg','image_06.jpg']
    return render_template('clients.html',navmenu=navmenu, imgs=imgs)

# Обработка запроса к странице contact.html
@app.route('/contact')
@login_required
def contact():
    return render_template('contact.html', navmenu=navmenu)
...
def login():
    ...
    if request.method == 'POST':
        ...
        return render_template('login.html', title='Whitesquare', navmenu=navmenu)
...
def register():
    ...
    if request.method == 'POST':
        ...
        return render_template('register.html', title='Whitesquare', navmenu=navmenu)
...
@app.route('/api/contactrequest/ivan', methods=['GET'])
# Получаем запись по имени пользователя
def get_contact_req_by_ivan():
    response = dbservice.get_contact_req_by_ivan()
    return json_response(response)
