...
# Получаем все запросы с именем Ivan
def get_contact_req_by_ivan():
    result = []
    rows = db.session.execute(f"SELECT * FROM contactrequests WHERE firstname = 'Ivan'").fetchall()
    for row in rows:
        result.append(dict(row))
    return {'contactrequests': result}
...
def create_contact_req(json_data):
    try:
        ...
        db.session.execute(f"INSERT INTO contactrequests "
                           f"(firstname, email, reqtype, reqtext, createdAt, updatedAt, ownerId) "
                           f"VALUES ("
        ...
        return {'message': "Ваш запрос обработан!"}
